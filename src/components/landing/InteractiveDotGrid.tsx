...
function DotFieldInner({ pointerRef, spacingPx, dotSize, color }: DotFieldInnerProps) {
  ...
  const geometry = useMemo(() => {
    ...
    g.setAttribute('position', new THREE.BufferAttribute(data.current, 3));
    return g;
  }, [data.current]);
  ...
  useFrame((state, delta) => {
    ...
    const influenceRadius = Math.max(viewport.width, viewport.height) * 0.085;
    ...
    for (let i = 0; i < data.count; i += 1) {
      ...
      if (pointer.active) {
        ...
        if (distance < influenceRadius && dotCooldown <= 0) {
          ...
          const impulse = (0.12 + pointerSpeed * 0.14) * t * repelStrength * data.jitter[i];
          velocityX += scatterX * impulse;
          velocityY += scatterY * impulse;
          life = Math.max(life, 0.72 + t * 0.2);
          dotCooldown = 0.09 + Math.random() * 0.18;
        }
      }
...
export function InteractiveDotGrid({
  ...
  useEffect(() => {
    const handleMove = (event: PointerEvent) => {
      ...
      pointerRef.current.speed = Math.min(2400, Math.hypot(vx, vy));
